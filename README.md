...
This repository holds our **Homework 1** submission materials: Part 1 (ML design write-up), Part 2 (Newswire fine-tuning + notebook), Part 3 (attention mechanisms notebook), and Part 4 (NOAA tornado analysis with HTML + PDF report).
...
| Path | What it is |
|------|------------|
| `homework 1/` | All assignment code, notebooks, and write-ups |
| `homework 1/finetune.py` | **PyTorch + Hugging Face `Trainer`** — DistilBERT, best on **GPU / Colab** |
| `homework 1/finetune_mlx.py` | **Apple MLX** fast path — BERT-base weights from Hugging Face Hub |
| `homework 1/BAX423_HW1_Part1_WriteUp.md` | Part 1 answers |
| `homework 1/BAX423_HW1_Part2_FineTuning.ipynb` | Part 2 notebook (run after training) |
| `homework 1/BAX423_HW1_Part3_AttentionMechanisms.ipynb` | Part 3 notebook |
| `homework 1/part4/tornado_analysis.py` | Part 4 pipeline |
| `homework 1/part4/output/` | Generated `tornado_report.html` and `tornado_report.pdf` |
| `homework 1/requirements.txt` | Python dependencies |
| `homework 1/BAX423_HW1_GRADING_CHECKLIST.txt` | Checklist vs. rubric |
...
### Part 2 — MLX (Apple Silicon, fast on-device)
...
3. **Why MLX helped**  
   In our setup, MPS was profiling around **~1–2 steps/s** for this assignment-style run (on the order of **many hours** for three epochs). MLX on Apple Silicon typically reaches **~5–15+ steps/s** for this pattern, so the **same hyperparameters** land in **roughly tens of minutes** instead of multi-hour runs. *Exact wall times belong in `part2_mlx_run*.log` after you complete a run.*  
   *Optional sanity check:* a **tiny smoke run** (few steps) can finish in **~2 minutes**; **full rubric training** uses the script defaults and takes longer—still far faster than MPS for us.
...
See `homework 1/BAX423_HW1_GRADING_CHECKLIST.txt`. Typical remaining items:
...
- **Part 4(e):** Email from a **non-UMN Gmail** (template in `homework 1/part4/`).  
